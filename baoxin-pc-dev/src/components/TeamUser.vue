  ...
  <div>
    <Modal v-model="getModal.isShow" width="430" @on-cancel="cancel">
      ...
      <div v-show="getModal.type =='login'" class="login-warp out-container">
        ...
        <p class="toAgent">
           <div class="toAgentLogin" @click="toAgentLogin">顾问登陆<Icon type="md-arrow-forward" /></div>
        </p>
      </div>
      <!-- register -->
...
export default {
  computed:{
    getModal(){
       return this.$store.state.showTeamUser
    }
  },
  mounted(){
    this.getLoginQrCode()
  ...
  methods: {
    cancel(){
       this.$store.commit('showTeamUser',{isShow:false})
    },
    // 获取登陆二维码
    getLoginQrCode() {
      // 获取当前网站协议 进行 动态切换地址
      ...
      var activeUrl = baseURL+location.pathname+"?roleType=2"
      var obj = new WxLogin({
        self_redirect: false,
        id: "login_container_team",
          ...
          this.$axios({
            ...
            .then(result => {
              if (result.data.code == 0) {
                ...
                this.$store.commit("loginInfo", { type: "team", isLogin: true ,phone:teamName || this.formLogin.user});
                // 登陆成功后跳转顾问页面
                this.$store.commit('showTeamUser',{isShow:false})
                setTimeout(function(){
                  _this.$router.push('/MangeTeam')
                },1000)
              } else {
    ...
    handlshowRegister() {
       this.$store.commit('showTeamUser',{isShow:true,type:'register'})
    },
    // 忘记密码提醒
    forget() {
